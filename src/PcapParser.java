...
public class PcapParser{
	...
	/* structure of captured packet: 
	 * Pcap header, Ethernet header, IP header, UDP/TCP header, data payload
	 */
	// Pcap header
	...
	// .pcap file, can be captured by Wireshark
	private FileInputStream fis;

	// Various typecast functions
	...
	// Get different protocols' header length
	...
	// Build IP/UDP/TCP packet based on captured data
	...
	private UDPPacket buildUDPPacket(byte[] packet, long timestamp) {
		...
		// Payload size = packet.length - payloadDataStart
	...
	private class PcapPacketHeader{
		...
		public static final int capLenOffset = 8; // read length starting from capLenOffset

		public long timestamp; // when is this packet captured
		public long packetSize; // total length of captured packet
	}
	...
	private PcapPacketHeader buildPcapPacketHeader(){
		...
		// total length of captured packtet, including protocol headers
	...
	// Get a packet from the pcap fle
